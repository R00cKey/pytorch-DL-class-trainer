...
class BaseDLFramework:
	def __init__(
		...
		best_model_save_path: str='best_model.pt'
			...
			self._epochs_run = 0 #The number of epochs which have been completed
			self._snapshot_path = snapshot_path
			self._model_init_path = model_init_path
			self._best_model_save_path = best_model_save_path
			self._best_train_loss = np.inf
			self.train_loss_by_epochs = [] #Save training loss in each epoch in order to plot train loss vs epochs
	...
	def load_init_model(self): ##Initialize the model defined in the class
	...
	def save_best_model(self): #Save the model which performed the best
		best_model={
		    "MODEL_STATE": self._model.state_dict(),
		    "VAL_LOSS": self._best_train_loss,
